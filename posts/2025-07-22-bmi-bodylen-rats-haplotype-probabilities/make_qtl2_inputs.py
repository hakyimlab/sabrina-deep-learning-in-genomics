...
def make_qtl_inputs(args):
    ...
    # command = f"{cntrl_command}; {qtl_command}"
    # escaped_command = command.replace("'", "'\"'\"'")

    with open(os.path.join(args.working_dir, "qtl2_calculate_prob.R"), "w") as f:
        f.write(f'{cntrl_command}\n{qtl_command}')


    os.chmod(os.path.join(args.working_dir, "qtl2_calculate_prob.R"), 0o755)

    # subprocess.run(f"cd {args.working_dir} && module use /soft/modulefiles && module load conda && unset LD_LIBRARY_PATH && conda run -n genomics R -e '{cntrl_command}'", shell=True)
